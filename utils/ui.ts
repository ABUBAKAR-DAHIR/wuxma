import { DesignNode, DesignSchema, Payload } from './../types/types';
export async function renderNode(node: DesignNode, parent: FrameNode) {
  if (node.type === "text") {
...
// ENGINE 
export async function renderDesign(design: DesignSchema) {
  const frame = figma.createFrame()
...
// export Demo
function generateLayout(payload: Payload): DesignSchema {
  return {
    frame: {
      width: 1200,
      height: 800,
